...
#define DEFAULT_MON_ADDR "127.0.0.1"
#define DEFAULT_MON_PORT 8080
...
static void usage(const char *progname) {
  fprintf(
    ...
    "\n"
    "  -L <addr>       management address (default " DEFAULT_MON_ADDR ")\n"
...
void client_parse_args(int argc, char **argv, struct client_args *args) {
  ...
  args->mon_addr = DEFAULT_MON_ADDR;
  args->mon_port = DEFAULT_MON_PORT;
  ...
  while ((c = getopt_long(argc, argv, "hvmUa:d:L:P:u:", long_options, NULL)) !=
    ...
    switch (c) {
      case 'h':
        ...
        exit(0);
      case 'v':
        fprintf(stdout, "mon_client 1.0\n");
        exit(0);
      case 'L':
        args->mon_addr = optarg;
        break;
      case 'P':
        args->mon_port = parse_port(optarg);
        break;
      case 'u':
