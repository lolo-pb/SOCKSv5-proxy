...
static size_t next_capacity(const size_t n) {
  ...
  assert(tmp >= n);
  if (tmp > ITEMS_MAX_SIZE) { tmp = ITEMS_MAX_SIZE; }
...
static void items_init(fd_selector s, const size_t last) {
  ...
  for (size_t i = last; i < s->fd_size; i++) { item_init(s->fds + i); }
}
...
static void items_update_fdset_for_fd(fd_selector s, const struct item *item) {
  ...
  if (ITEM_USED(item)) {
    if (item->interest & OP_READ) { FD_SET(item->fd, &(s->master_r)); }

    if (item->interest & OP_WRITE) { FD_SET(item->fd, &(s->master_w)); }
  }
}
  ...
  if (s != NULL) {
    if (s->fds != NULL) {
      for (size_t i = 0; i < s->fd_size; i++) {
        if (ITEM_USED(s->fds + i)) { selector_unregister_fd(s, i); }
      }
...
selector_status selector_register(
  ...
  if (ufd > s->fd_size) {
    ...
    if (SELECTOR_SUCCESS != ret) { goto finally; }
  }
...
selector_status selector_select(fd_selector s) {
  ...
  int fds =
    pselect(s->max_fd + 1, &s->slave_r, &s->slave_w, 0, &s->slave_t, &emptyset);
  if (-1 == fds) {
    switch (errno) {
      case EAGAIN:
      case EINTR:
  ...
  if (ret == SELECTOR_SUCCESS) { handle_block_notifications(s); }
finally:
  return ret;
}
...
int selector_fd_set_nio(const int fd) {
  ...
  if (flags == -1) {
    ...
    if (fcntl(fd, F_SETFL, flags | O_NONBLOCK) == -1) { ret = -1; }
  }
  return ret;
}
